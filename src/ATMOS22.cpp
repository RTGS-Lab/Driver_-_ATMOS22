...
ATMOS22::ATMOS22(SDI12Talon& talon_, uint8_t talonPort_, uint8_t sensorPort_, uint8_t version): talon(talon_)
{
...
String ATMOS22::begin(time_t time, bool &criticalFault, bool &fault)
{
	// Serial.println("ATMOS22 - BEGIN"); //DEBUG!
...
bool ATMOS22::isPresent() 
{ //FIX!
	// Wire.beginTransmission(0x77);
	// int errorA = Wire.endTransmission();
	// Wire.beginTransmission(0x76);
	// int errorB = Wire.endTransmission();
	// Serial.print("ATMOS22 TEST: "); //DEBUG!
	// Serial.print(errorA);
	// Serial.print("\t");
	...
	Serial.print(",");
	Serial.println(id);
	if(id.indexOf("ATM22") > 0) return true; //FIX! Check version here!
	// if(errorA == 0 || errorB == 0) return true;
	else return false;
}

int ATMOS22::indexOfSep(String input)
{
...
// void ATMOS22::setTalonPort(uint8_t port)
// {
...
// void ATMOS22::setSensorPort(uint8_t port)
// {
// 	// if(port_ > numPorts || port_ == 0) throwError(PORT_RANGE_ERROR | portErrorCode); //If commanded value is out of range, throw error 
